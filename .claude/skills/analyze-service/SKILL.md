...
description: "GLUE 프레임워크 기반 레거시 시스템 전체 분석 자동 실행 (Phase 1-5 통합 오케스트레이션). Phase 1(구조 파악), Phase 2(Java 분석), Phase 2.5(커스텀 클래스), Phase 3(SQL 분석), Phase 4(UI 분석), Phase 5(통합 문서 생성)를 순차 실행. 사용 시점: /analyze-service SERVICE-ID 호출 시, 레거시 서비스 전체 분석이 필요할 때, 서비스 ID(예: M473020030, B47R1001)에 대한 종합 분석 요청 시. 개별 Phase만 실행 요청도 지원 (예: Phase 3만 실행해줘)"
---

# 레거시 시스템 전체 분석

GLUE Service XML 기반 레거시 서비스를 Phase 1~5 순차 실행하여 비즈니스 로직, 데이터 구조, UI 요구사항을 추출하고 상세 요구사항 문서를 생성한다.
...
6. Phase 5: 통합 문서 생성 → [references/phase5.md](references/phase5.md)
...
- Phase 5만 → [references/phase5.md](references/phase5.md) 참조
...
> **자동 스킵**: Phase 1~4 스크립트는 출력 JSON이 이미 존재하면 자동 스킵한다 (exit 0). `--force` 플래그 추가 시 중간 JSON을 무시하고 전체 Phase를 재실행한다. **Phase 5(통합 문서)는 항상 (재)생성한다** — 중간 JSON이 갱신될 수 있으므로 최종 문서는 매 실행마다 새로 작성한다.

### 시간 추적

각 Step 시작 전에 Bash로 `date '+[%H:%M:%S]'`를 실행하여 시작 시각을 출력한다. 최초 Step 1 시작 시각을 기록하고, Step 7 완료 보고에서 전체 경과 시간을 함께 표시한다.
...
### Step 6: Phase 5 - 통합 문서 생성

Bash로 시작 시각 출력: `echo "⏱️ Step 6 (Phase 5 통합 문서) 시작: $(date '+%H:%M:%S')"`
...
완료 후: `echo "✅ Step 6 완료: $(date '+%H:%M:%S')"`

### Step 6.5: PL/SQL 자동 분석 및 보고서 갱신

Bash로 시작 시각 출력: `echo "⏱️ Step 6.5 (PL/SQL 분석) 시작: $(date '+%H:%M:%S')"`
...
- `plsqlCalls` 섹션 없음 또는 `detectedCalls` 비어있음 → Step 7로 진행
- 미분석 항목 0개 → Step 7로 진행
- 미분석 항목 있음 → 6.5-1a로 진행
...
#### 6.5-1a. PL/SQL 소스 일괄 조회 (DB 1회 접속)
...
완료 후: `echo "✅ Step 6.5 완료: $(date '+%H:%M:%S')"`

### Step 7: 완료 보고

Bash로 완료 시각 출력: `echo "⏱️ 전체 분석 완료: $(date '+%H:%M:%S')"`

```
📊 통합 결과:
- Activities: [N]개, Custom 클래스: [M]개, SQL 쿼리: [K]개, UI 컴포넌트: [L]개
- PL/SQL: [감지]개 감지, [분석완료]개 분석 완료
- 전체 소요 시간: [Step 1 시작 ~ Step 7 완료 경과 시간]

📁 최종 출력: docs/analysis/service/[ui|nui]/[SERVICE-ID]_legacy_analysis.md
```
