...
public class C10UiCheckActivity3 extends PosActivity
{
    ...
    public String runActivity( PosContext ctx )
    {
        ...
        try
        {
            ...
            { // 작업지시변경저장 coil
                ...
                String[] ORD_NO;
                String[] ORD_LN;
                String[] ORD_BAK_SND_CAU;                
                String sORD_NO;
                String sORD_LN;
                String sORD_BAK_SND_CAU;
                int nRELULT = 0;
                ...
                for ( int i = 0, n = idsValue.length; i < n; i++ )
                {
                    ...
                    ORD_BAK_SND_CAU = (String[]) ctx.get( idsValue[i] + 
                            C10ConstantsIF.UNDERBAR + C10ConstantsIF.ORD_BAK_SND_CAU );                    

                    sORD_NO = ORD_NO[0];
                    sORD_LN = ORD_LN[0];
                    sORD_BAK_SND_CAU = ORD_BAK_SND_CAU[0];                    
                    ...
                    if ( ORD_NO.length > 0 && ORD_LN.length > 0 )
                    {
                        ...
                        rowset = mesdao.find( C10ConstantsIF.C104000050_CHECK3, param );

                        logger.logDebug( C10ConstantsIF.SELECT_COUNT + rowset.count() );
                        if ( rowset.count() == 0 )
                        {
                            // error
                            logger.logError( C10ConstantsIF.C104000050_CHECK_ERR3 );
                            // throw new PosException( C10ConstantsIF.C104000050_CHECK_ERR );

                            ctx.put( C10ConstantsIF.ERRMSG, C10ConstantsIF.C104000050_CHECK_ERR3 );

                            return PosBizControlConstants.FAILURE;

                        }
                        /*
                         * select ORD_BAK_SND_TP
                         * FROM TB_C10_QLT_DSN_CMN
                         * WHERE ORD_NO = :ORD_NO
                         * AND ORD_LN = :ORD_LN
                         * AND ORD_BAK_SND_TP = 'R'
                         */

                        param = new PosParameter();
                        param.setValueParamter( C10ConstantsIF.ORD_NO, sORD_NO );
                        param.setValueParamter( C10ConstantsIF.ORD_LN, sORD_LN );
                        param.setNamedParamter( C10ConstantsIF.ORD_BAK_SND_CAU, sORD_BAK_SND_CAU );                        
                        ...
                        nRELULT = mesdao.update( C10ConstantsIF.C104000050_UPDATE3, param );
                        logger.logDebug( C10ConstantsIF.C104000050_UPDATE3 );
                        ...
                        param.setNamedParamter( C10ConstantsIF.COL_QLT_DSN_STS, C10ConstantsIF.C );
                        param.setNamedParamter( C10ConstantsIF.COL_QLT_DSN_MSG, sORD_BAK_SND_CAU );
                        param.setNamedParamter( C10ConstantsIF.OBJECT_TYPE, 
                                (String) ctx.get( C10ConstantsIF.OBJECT_TYPE ) );
                        param.setNamedParamter( C10ConstantsIF.OBJECT_ID, 
                                (String) ctx.get( C10ConstantsIF.OBJECT_ID ) );
