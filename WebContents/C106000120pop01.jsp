...
<%--
 * PROGRAM NAME     :  C106000120pop01.jsp
 * VERSION          :  V1.0
 * DESCRIPTION      :  영문 보증서 파일 업로드
 * DESIGNER NAME    :  이 돈 석
 ...
 * ------------  ------    --------  ------------------------
 * 2020.09.28     V1.0      이돈석      Initial Version
--%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
String WAR_PRT_SEQ_NO       = request.getParameter("WAR_PRT_SEQ_NO");
String CCL_BOM_NO 	        = request.getParameter("CCL_BOM_NO");
String CCL_BOM_WTY_SEQ_NO 	= request.getParameter("CCL_BOM_WTY_SEQ_NO");
...
var pageConfiguration = '[' + 
      '{"itemType":"grid","renderTo":"C106000120pop01_Grid_1","xml":".\/header\/kr\/C106000120pop01\/C106000120pop01_Grid_1.xml","rowCnt":"2","vertical":"true","url":"gridC10Data.do","contextmenu":"false","borderline":"true","pageset":"true","split":"0","referenceItem":"C106000120pop01_Form_1","service":"C106000120pop01-service","actionType":"save"},' +
      '{"itemType":"messagebox","renderTo":"C106000120pop01_MessageBox_1","xml":".\/header\/kr\/C106000120pop01\/C106000120pop01_MessageBox_1.xml","service":"C106000120pop01-service"}' +
   ']';
...
function findMessage(referenceItem){
	uiCommon.message("C106000120pop01_MessageBox_1",referenceItem.getUserData("","appMsg"));
  	return true;
}
var vault = null;
function onVaultLoad() {
	...
	vault.setServerHandlers("_uploadHandler4.jsp", "_getInfoHandler.jsp", "_getIdHandler.jsp");	
	
	vault.create("vault1");
	vault.setFormField("IMG_RGS_TP"		, "<%=IMG_RGS_TP%>");
	vault.setFormField("IMG_RGS_FLAG"	, "07");
	vault.setFormField("IMG_RGS_FLAG_ID" , "<%=WAR_PRT_SEQ_NO%>");
	vault.setFormField("IMG_RGS_FLAG_ID1", "<%=CCL_BOM_NO%>");	
	vault.setFormField("IMG_RGS_FLAG_ID2","<%=CCL_BOM_WTY_SEQ_NO%>");
	vault.setFormField("PRD_SPC_TP"      ,"<%=PRD_SPC_TP%>");
	
	/* 모든 파일이 다 등록 될 수 있게 프로그램 변경
...
function onLoadGrid(){
	var grid = items["C106000120pop01_Grid_1"];
	var customparam = {"WAR_PRT_SEQ_NO":"<%=WAR_PRT_SEQ_NO%>","CCL_BOM_NO":"<%=CCL_BOM_NO%>"};	
	var findUrl  = parametersC10('C106000120pop01_Grid_1','find',customparam);
	items['C106000120pop01_Grid_1'].loadData(findUrl);
	grid.getDhxGrid().detachEvent(onXleGrid);
}

function Grid_doLink(filename,rowId){
	var gridObj = items["C106000120pop01_Grid_1"].getDhxGrid();
...
function doImgDel(rowIdx){
	var gridObj = items["C106000120pop01_Grid_1"].getDhxGrid();
	var WAR_PRT_SEQ_NO   	= gridObj.cellById(gridObj.getRowId(rowIdx),gridObj.getColIndexById("WAR_PRT_SEQ_NO")).getValue();
	var CCL_BOM_NO			= gridObj.cellById(gridObj.getRowId(rowIdx),gridObj.getColIndexById("CCL_BOM_NO")).getValue();
	var CCL_BOM_WTY_SEQ_NO	= gridObj.cellById(gridObj.getRowId(rowIdx),gridObj.getColIndexById("CCL_BOM_WTY_SEQ_NO")).getValue();
	...
	del_url += "?IMG_RGS_FLAG=07";
	del_url += "&IMG_RGS_FLAG_ID="+WAR_PRT_SEQ_NO;
	del_url += "&IMG_RGS_FLAG_ID1="+CCL_BOM_NO;
	del_url += "&IMG_RGS_FLAG_ID2="+CCL_BOM_WTY_SEQ_NO;
	del_url += "&SEQ="+SEQ;
...
<div id="vault1"></div>
<div id="C106000120pop01_Grid_1" style="position:absolute;height:70px;width:430px;left:8px;top:260px;">
</div>
<div id="C106000120pop01_MessageBox_1" style="position:absolute;height:23px;width:445px;left:0px;top:332px;">
</div>
</body>
</html>
<script>
...
//<![CDATA[
   ui.initializeDHTMLX();  
   var onXleGrid = items["C106000120pop01_Grid_1"].onXLEEvent(onLoadGrid);     
//]]>
-->
</script>
