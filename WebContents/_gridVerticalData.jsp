...
try
{
	...
	if(ctx.get("column-info") == null){
		out.println("<row>");
		String service = request.getParameter("ServiceName");
		String message = "Parameter [column-info] is null";     
		throw new Exception(message);
	}
	
	/* column-info는 AAA,BBB,CCC|EEE,FFF,GGG|KKK,LLL,MMM 과 같은 형태로 받는다 */
	String [] columnInfo = ((String) ctx.get("column-info")).split("\\|");
	...
	if(resultKey != null) {
		...
		if(result != null ){
			...
			userdata.append("<rows>")
					.append("<userdata name=\"appMsg\">")
					.append((String)ctx.get("appMsg"))
					.append("</userdata>");
			out.print(userdata.toString()); 
//			System.out.println(userdata.toString());
			
			/* List인 경우와 PosRowSet인 두 가지 경우에 대해 처리한다. */
			if (result instanceof PosRowSet ){
				if(((PosRowSet)result).hasNext()){

					List rowSetList = (List)result;
					
					PosRow row = (PosRow) rowSetList.get(0);	// 한 건의 Data에 대해서만 처리 가능하다

					Object cellData = null;
					
					String[] columns;
					for(int i = 0, size = columnInfo.length; i < size; i++){
						out.println("<row id=\"" + i + "\">");
//						System.out.println("<ROW id=\"" + i + "\">");
						
						columns = columnInfo[i].split(",");
						
						StringBuffer sb = new StringBuffer();
						Object value = null;
						for(int j = 0; j < columns.length; j++) {
							sb.append("<cell id=\"").append(columns[j]).append("\"><![CDATA[");
							value = row.getAttribute(columns[j]);
