...
<%--
 * PROGRAM NAME     :  C106000160.jsp
 * VERSION                 :  1.0
 * DESCRIPTION          :  Keyword관리
 * DESIGNER NAME     :  이돈석                              
 * DEVELOPER NAME  :  이돈석
...
<%
    ...
    String KEY_WRD = request.getParameter("KEY_WRD") 	!= null ? request.getParameter("KEY_WRD") 	: "";
%>    
...
var pageConfiguration = '[' + 
      '{"itemType":"form","renderTo":"C106000160_Form_1","xml":".\/header\/kr\/C106000160\/C106000160_Form_1.xml","url":"gridC10Data.do","referenceItem":"C106000160_Grid_1","service":"C106000160-service","actionType":"save","security":"true"},' +
      '{"itemType":"menu","renderTo":"C106000160_Menu_1","xml":".\/header\/kr\/C106000160\/C106000160_Menu_1.xml","iconImgs":".\/dhtmlx\/codebase\/imgs\/","referenceItem":"C106000160_Grid_1","service":"C106000160-service"},' +
      '{"itemType":"grid","renderTo":"C106000160_Grid_1","xml":".\/header\/kr\/C106000160\/C106000160_Grid_1.xml","rowCnt":"22","vertical":"true","url":"handleDataProcess.do","contextmenu":"true","borderline":"true","pageset":"true","split":"0","referenceItem":"C106000160_Grid_1","service":"C106000160-service","actionType":"save"},' +
      '{"itemType":"messagebox","renderTo":"C106000160_messagebox","xml":".\/header\/kr\/C106000160\/C106000160_messagebox.xml","service":"C106000160-service"}' +
   ']';
...
function save(eventName,formDivObj,referenceItem){
    var gridObj = items['C106000160_Grid_1'].getDhxGrid();
	...
	var rowId = items['C106000160_Grid_1'].getRowSelectedId();
	...
	for(var i=0; i< grid_cnt; i++){
		...
		if(row_status !== ""){
			...
			if(row_status=="inserted"){
				/*
				KEY_WRD = gridObj.cellById(gridObj.getRowId(i),1).getValue();
								
				if(KEY_WRD == "" ){
					err_cnt++;
					dhtmlx.alert("키워드를 입력하세요.");
					break;
				}
				*/
			}
...
function doImgPopUp8(rowIdx){	
	var gridObj = items['C106000160_Grid_1'].getDhxGrid();
	var rowId = gridObj.getRowIndex(rowIdx);
	
	var md_url = "C106000160pop01.jsp?rowId="+rowId;
		md_url += "&KEY_WRD_NO="+items['C106000160_Grid_1'].getCellByIndexValue(rowId,gridObj.getColIndexById('KEY_WRD_NO'));
		md_url += "&KEY_WRD_SEQ_NO="+items['C106000160_Grid_1'].getCellByIndexValue(rowId,gridObj.getColIndexById('KEY_WRD_SEQ_NO'));
		md_url += "&parent_item=C106000160_Grid_1";
		md_url += "&IMG_RGS_TP=1";  // 1로 고정(PC)
		md_url += "&PRD_SPC_TP=7";  // 7로 고정(파일이미지)
		
	var cusWinObj = new ui.window("C106000160PopWin","키워드 이미지 등록","0","0","465","405",md_url);
...
function doImgPopUp9(rowIdx){	
	var gridObj = items['C106000160_Grid_1'].getDhxGrid();
	var rowId = gridObj.getRowIndex(rowIdx);
	
	var md_url = "C106000160pop02.jsp?rowId="+rowId;
		md_url += "&KEY_WRD_NO="+items['C106000160_Grid_1'].getCellByIndexValue(rowId,gridObj.getColIndexById('KEY_WRD_NO'));
		md_url += "&KEY_WRD_SEQ_NO="+items['C106000160_Grid_1'].getCellByIndexValue(rowId,gridObj.getColIndexById('KEY_WRD_SEQ_NO'));
		md_url += "&parent_item=C106000160_Grid_1";
		md_url += "&IMG_RGS_TP=1";  // 1로 고정(PC)
		md_url += "&PRD_SPC_TP=8";  // 8로 고정(규격이미지)
		
	var cusWinObj = new ui.window("C106000160PopWin","키워드 파일 등록","0","0","465","405",md_url);
	cusWinObj.setButtonDisable("park,minmax1");
	cusWinObj.setModal();
}
...
function doOnRowClicked(rId,cInd) {
	/*
	var grid  = items['C106000160_Grid_1'];
	...
	if(cInd == 2){
		...
		winObj = new ui.window("popup2","고객사","0","0","469","532","masterGridData.do?CD_TP=CUS_CD&CATEGORY_GROUP_NM=SZ0000&targetName="+cInd+"&targetFormID=C106000160_Grid_1&CD_V="+cellVal);
...
function masterSetValue(code,name,target,targetDivId){
	if(targetDivId.indexOf('Grid') > -1){
		...
		if(target == "2"){		
			...
			items['C106000160_Grid_1'].setUpdated(rowId,true,"updated");
		}else{
	...
	if(stage==1) {
		...
		if(btnChk == false ) {
			if(cInd == 2){
				...
				{
					winObj = new ui.window("popup2","고객사","0","0","469","532","masterGridData.do?CD_TP=CUS_CD&CATEGORY_GROUP_NM=SZ0000&targetName="+cInd+"&targetFormID=C106000160_Grid_1&CD_V_MEANING="+cellVal);
				}
				*/
				winObj.setButtonDisable("park,minmax1");
				winObj.setModal();
			...
			if(cInd==3 && !isNull(nValue)){
				...
				*/
				...
				var param= "ServiceName=C106000160-service&colorFind=1&CCL_BOM_NO="+nValue+"&column-info=CCL_BOM_NO";
...
function refresh(referenceItem){ //grid selection clear event
    ...
    var findUrl = uiCommon.parameters('C106000160_Form_1',referenceItem,'find');
    items[referenceItem].loadData(findUrl);	
}
...
function setAutoData(id,cInd){
	var grdObj = items['C106000160_Grid_1'].getDhxGrid();
	if(cInd==0){
		var row_status = grdObj.getUserData(id,"!nativeeditor_status");
		if(row_status=="inserted"){
			
			grdObj.cellById(id,grdObj.getColIndexById("KEY_WRD_DH")).setValue(uiCommon.getCurrentDate());
		}
	}
}
...
function remove(referenceItem){
	...
	if(items['C106000160_Grid_1'].getSelectedRowId()==null){
...
function findMessage(referenceItem){
	uiCommon.message("C106000160_messagebox",referenceItem.getUserData("","appMsg"));
  	return true;
}
...
function parentViewImg(rowIdx,imgnm){
	...
	var md_url = "C106000080pop02.jsp?grid=C106000160_Grid_1&imgnm="+encodeURIComponent(imgnm)+"&img_rgs_flags=04";
...
function onAfterUpdateFinishEvent(){
	//var msg = getMessage('C106000160_messagebox');
	//if (msg != "") {
	  //items['C106000160_Form_1'].setItemValue('saveMessage',msg);
	  //find('find','C106000160_Form_1','C106000160_Grid_1');
	  refresh('C106000160_Grid_1');
	//}
}
...
function firstFind(){
	...
	items['C106000160_Form_1'].setItemValue("KEY_WRD","<%=KEY_WRD%>");			
	var parentFindUrl = uiCommon.parameters('C106000160_Form_1','C106000160_Grid_1','find');
	items["C106000160_Grid_1"].loadData(parentFindUrl);	
	uiCommon.progressOff(parent);	
	
	items['C106000160_Grid_1'].getDhxGrid().detachEvent(grdXle);
}


//그리드안의 주문번호 클릭 시 링크 연결 
function doOnRowDblClicked(rowId) {
	//var CCL_BOM_NO = items['C106000160_Grid_1'].getDhxGrid().cells(rowId,0).getValue();
...
<body>
<div id="C106000160_Form_1" style="position:absolute;height:28px;width:981px;left:0px;top:0px;">
</div>
<div id="C106000160_Menu_1" style="position:absolute;height:25px;width:981px;left:0px;top:28px;">
</div>
<div id="C106000160_Grid_1" style="position:absolute;height:511px;width:980px;left:-1px;top:54px;">
</div>
<div id="C106000160_messagebox" style="position:absolute;height:19px;width:980px;left:-1px;top:567px;">
</div>
</body>
</html>
<script>
...
//<![CDATA[
	ui.initializeDHTMLX();
	items['C106000160_Grid_1'].onAfterUpdateFinishEvent(onAfterUpdateFinishEvent);
	items['C106000160_Grid_1'].onXLEEvent(setReadonly);
	var grdXle = items['C106000160_Grid_1'].onXLEEvent(firstFind);
	items["C106000160_Grid_1"].onEditCellEvent(onEditCellEvent);
	items["C106000160_Grid_1"].rowDblClicked(doOnRowClicked);
	//items["C106000160_Grid_1"].onEditCellEvent(gridChangedEvent);
	//items['C106000160_Grid_1'].getDhxGrid().attachEvent("onEditCell",gridChangedEvent); //grid cell Edit event
	// 행삭제시 삭제데이타 붉은색 실선으로 표시
	var dataProcessor = items["C106000160_Grid_1"].getDhxDataProcess();
	//items["C106000160_Grid_1"].rowDblClicked(doOnRowDblClicked);
