...
function eXcell_ahref(cell){       
  /** default pattern, just copy it */
	...
	this.setValue=function(val){
	  ...
	  this.setCValue("<a href='javascript:void(0)' onclick='doLink(\""+val+"\")' onFocus='blur()'>"+val+"</a>",val);                                      
	},
...
eXcell_ahref.prototype = new eXcell;    

/**
 * @class DHTMLX custom Form Button Function  
 * @constructor
 * @see ttp://docs.dhtmlx.com/doku.php?id=dhtmlxform:custom_item
 */ 
dhtmlXForm.prototype.items.custombutton = {
	
	render: function(item, data) {
		
		item._type = "custombutton";
		item._enabled = true;
		...
		item.innerHTML = '<div class="dhx_list_btn_custom" role="link" tabindex="0" dir="ltr" '+
					...
					'<table cellspacing="0" cellpadding="0" border="0" align="left">'+
						'<tr>'+
							'<td class="custom_btn_l"><div class="custom_btn_l">&nbsp;</div></td>'+
							'<td class="custom_btn_m"><div class="btn_txt"'+(w!=null?' style="width:'+w+'px;"':'')+'>'+data.value+'</div></td>'+
							'<td class="custom_btn_r"><div class="custom_btn_r">&nbsp;</div></td>'+
						'</tr>'+
					'</table>'+
				"</div>";
		...
		item.childNodes[0].childNodes[0].onmouseover = function(){
			...
			this._isOver = true;
			this.className = "dhx_list_btn_custom_over";
			t = null;
		},
		...
		item.childNodes[0].childNodes[0].onmousedown = function(){
			...
			var t = this.parentNode.parentNode;
			if (!t._enabled) return;
			this.className = "dhx_list_btn_custom_pressed";
...
/**
 * @class DHTMLX link Form Button Function  
 * @constructor
 * @seeh ttp://docs.dhtmlx.com/doku.php?id=dhtmlxform:custom_item
 */ 
dhtmlXForm.prototype.items.linkbutton = {
	
	render: function(item, data) {
		
		item._type = "linkbutton";
		item._enabled = true;
		item._cmd = data.command;
		item._name = data.name;
		...
		item.innerHTML = '<div class="dhx_list_btn_link" role="link" tabindex="0" dir="ltr" '+
					'onkeypress="e=event||window.arguments[0];if((e.keyCode==32||e.charCode==32)&&!this.parentNode._busy){this.parentNode._busy=true;e.cancelBubble=true;e.returnValue=false;_dhxForm_doClick(this.childNodes[0],[\'mousedown\',\'mouseup\']);return false;}" '+
					'ontouchstart="e=event;e.preventDefault();if(!this.parentNode._busy){this.parentNode._busy=true;_dhxForm_doClick(this.childNodes[0],[\'mousedown\',\'mouseup\']);}" '+
					'onblur="_dhxForm_doClick(this.childNodes[0],\'mouseout\');" >'+
					'<table cellspacing="0" cellpadding="0" border="0" align="left">'+
						'<tr>'+
							'<td class="link_btn_l"><div class="link_btn_l">&nbsp;</div></td>'+
							'<td class="link_btn_m"><div class="btn_txt"'+(w!=null?' style="width:'+w+'px;"':'')+'>'+data.value+'</div></td>'+
							'<td class="link_btn_r"><div class="link_btn_r">&nbsp;</div></td>'+
						'</tr>'+
					'</table>'+
				"</div>";
		...
		item.childNodes[0].childNodes[0].onmouseover = function(){
			...
			if (!t._enabled) return;
			this._isOver = true;
			this.className = "dhx_list_btn_link_over";
			t = null;
		},
		item.childNodes[0].childNodes[0].onmouseout = function(){
			var t = this.parentNode.parentNode;
		...
		item.childNodes[0].childNodes[0].onmousedown = function(){
			...
			var t = this.parentNode.parentNode;
			if (!t._enabled) return;
			this.className = "dhx_list_btn_link_pressed";
			this._allowClick = true;
			this._pressed = true;
			t = null;
		},
...
/**
 * @class DHTMLX custom Form calendar Function  
 * @constructor
 */ 
...
dhtmlXForm.prototype.items.calendar = {
		...
		getValue: function(item, asString) {
			var _formatOption = (typeof asString == 'undefined')?true:asString;
		  return (_formatOption===true?this.getFValue(item, item._value, this.f0[item._idd]):item._value);
		},
...
dhtmlXGridObject.prototype._OnSelectionStart=function(a){var b=this;if(a.button!=2){var c=a.srcElement||a.target;if(this.editor){if(c.tagName&&(c.tagName=="INPUT"||c.tagName=="TEXTAREA" ||c.tagName=="IMG"))return;this.editStop()}b.isActive||b.setActive(!0);var d=this.getPosition(this.obj),e=a.clientX-d[0]+document.body.scrollLeft,f=a.clientY-d[1]+document.body.scrollTop;this._CreateSelection(e-4,f-4);if(c==this._selectionObj)this._HideSelection(),this._startSelectionCell=null;else{for(;c.tagName.toLowerCase()!="td";)c=
...
/**
 * @class DHTMLX custom Form item Function  
 * @constructor
 */ 
function setTitleIcon(name,val){
	return "<img src="+window.dhx_globalImgPath+val+".gif align='top'>";
}

/** dhtmlx close button Function */
function winClose(){
    if(typeof(parent.winObj) !== 'undefined'){
	parent.winObj.winClose();					
    }else{
		if(typeof(parent.tabClose) !== 'undefined'){
    		parent.tabClose();
		}
	}
}

/** dhtmlx grid sort Function */
