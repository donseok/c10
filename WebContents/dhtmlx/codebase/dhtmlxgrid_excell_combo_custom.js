...
*	@변졍전: value = this._combo_pre+"<div style='float:left;width:"+(this.cell.offsetWidth-this._combo_pre_width)+"px;line-height:"+(this.cell.offsetHeight-1)+"px;overflow:hidden'>"+value+"</div>";
*	@변경후: value = this._combo_pre+"<div style='margin-top:3px;float:left;width:"+(this.cell.offsetWidth-this._combo_pre_width)+"px;line-height:"+(this.cell.offsetHeight-5)+"px;overflow:hidden'>"+value+"</div>";
...
eXcell_combo_v=function(cell){
    ...
    combo._combo_pre = "<img src='"+(window.dhx_globalImgPath?window.dhx_globalImgPath:this.grid.imgURL)+"combo_select_grid"+".gif' style='float:right;"+(_isIE?"margin-right:-3px;":"")+"'/>";
...
eXcell_combo.prototype.initCombo = function(index){
	...
	var selIndex = combo.getSelectedIndex();
	combo.selectOption(selIndex, true, true);	
	combo.attachEvent("onKeyPressed",function(ev){setTimeout(function(){if(ev==13||ev==27) {grid.editStop();if(grid._fake) grid._fake.editStop()}},5);});
	grid.attachEvent("onScroll", function(sLeft,sTop){grid.editStop();if(grid._fake) grid._fake.editStop()});
	dhtmlxEvent(combo.DOMlist,"click",function(){setTimeout(function(){grid.editStop();if(grid._fake) grid._fake.editStop();},5);});
...
eXcell_combo.prototype.setComboCValue = function(value,value2){
   	if(this._combo_pre!="")
   		value = this._combo_pre+"<div style='padding-top:1px; margin-top:2px;float:left;width:"+(this.cell.offsetWidth-this._combo_pre_width)+"px;line-height:"+(this.cell.offsetHeight-5)+"px;overflow:hidden'>"+value+"</div>";
