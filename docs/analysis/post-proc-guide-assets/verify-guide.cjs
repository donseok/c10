...
const file=path.join(base,'후공정_제조표준_설계_쉽게이해하기.html');
...
(async()=>{
 ...
 const n=await page.locator('#pp-scenario option').count();
 assert.equal(n,6,'6 scenarios');
 const seen=new Set();
 for(let i=0;i<n;i++){
  const value=await page.locator('#pp-scenario option').nth(i).getAttribute('value');
  await page.selectOption('#pp-scenario',value);
  const steps=await page.locator('#pp-steps li').count();
  assert.equal(steps,4,'4 steps for '+value);
  const state=await page.locator('#pp-result').getAttribute('data-state');
  assert.ok(['shown','hidden','terminated'].includes(state),'state set for '+value);
  assert.ok((await page.locator('#pp-result').innerText()).length>0,'Summary for '+value);
  assert.ok(await page.locator('#pp-rows tr').count()>=8,'Condition rows for '+value);
  seen.add(value);
 }
 assert.equal(seen.size,6,'All scenarios distinct');
 await page.selectOption('#pp-scenario','coil-st');
