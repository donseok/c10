...
const file=path.join(base,'용융_전기_제조표준_설계_쉽게이해하기.html');
...
(async()=>{
 ...
 assert.equal(check.imgs.length,14,'14 figures rendered');
 ...
 const n=await page.locator('#mnf-scenario option').count();
 assert.equal(n,6,'6 scenarios');
 const seen=new Set();
 for(let i=0;i<n;i++){
  const value=await page.locator('#mnf-scenario option').nth(i).getAttribute('value');
  await page.selectOption('#mnf-scenario',value);
  const steps=await page.locator('#mnf-steps li').count();
  assert.equal(steps,4,'4 steps for '+value);
  const set=await page.locator('#mnf-result').getAttribute('data-set');
  assert.ok(/^\d\.\d{3}$/.test(set),'X-Ray SET formatted for '+value);
  assert.ok((await page.locator('#mnf-result').innerText()).length>0,'Summary for '+value);
  assert.ok(await page.locator('#mnf-rows tr').count()>=8,'Condition rows for '+value);
  seen.add(value);
 }
 assert.equal(seen.size,6,'All scenarios distinct');
 await page.selectOption('#mnf-scenario','gi');
