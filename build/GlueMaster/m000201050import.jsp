...
/*
 * @FileName      : 마스타코드 목록 >> 마스타코드 - 카테고리 관리 >> 마스타코드 - 카테고리별 코드값 관리 >> [import] 마스타코드 - 카테고리별 코드값 관리
 * Open Issues    :
 * Change history 
...
<title>[import] <%=PosContext.getResourceMessage("GMResource","gm.title.033",null,locale)%></title>
  ...
  <tr>
    <td valign=top>
      <table width="460" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr height=25>
          <td><div id=gm_title style="font-family:Verdana;font-size:16px;font-weight:bold;color:000033">[import] <%=PosContext.getResourceMessage("GMResource","gm.title.033",null,locale)%></div></td>
        </tr>
        <tr>
          <td background="img/gm0006img.gif"><img src="img/gm0007img.gif" width=1 height=8></td>
        </tr>
<form name="form_import" method="post" enctype="multipart/form-data" action="m000201050.do">
...
<input type="hidden" name="CdTpId" value="<%=request.getParameter("CdTpId")%>">
<input type="hidden" name="CdTp" value="<%=request.getParameter("CdTp")%>">
<input type="hidden" name="CdTpMeaning" value="<%=request.getParameter("CdTpMeaning")%>">
<input type="hidden" name="LanguageCode" value="<%=request.getParameter("LanguageCode")%>">
</form>
</BODY>
</HTML>