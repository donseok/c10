...
/*
 * @FileName      : 마스타코드 목록 >> 마스타코드 - 카테고리 관리 >> 마스타코드 카테고리 목록 PopUp화면
 * Open Issues    :
 * Change history 
...
<title><%=PosContext.getResourceMessage("GMResource","gm.title.032",null,locale)%></title>
...
<input type="hidden" name="LocCategoryGroupExplain" value="<%=request.getParameter("LocCategoryGroupExplain")%>">
      <table width="460" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr height=25>
          <td><div id=gm_title style="font-family:Verdana;font-size:16px;font-weight:bold;color:000033"><%=PosContext.getResourceMessage("GMResource","gm.title.032",null,locale)%></div></td>
        </tr>
        <tr>
  ...
  <img src="<%=PosContext.getResourceMessage("GMResource","gm.img.find",null,locale)%>" onClick="gm_find()" style="cursor:pointer" align="absmiddle">
                ...
                <td align="right">
                  <img src="<%=PosContext.getResourceMessage("GMResource","gm.img.ok",null,locale)%>" onClick='javascript:returnAttrs();' style="cursor:pointer"><!-- confirm -->
  ...
  <table id="category" width="450" border=1 cellspacing="0" cellpadding="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
    ...
    PosRowSet rowset=(PosRowSet)ctx.get("CategoryGrResult");
    int rowCnt = 0;
...
%>
    <tr index=<%=rowCnt%> class=<%=(rowCnt%2==1?"tblcg":"tblcw")%> height=20>
      <td><input type="radio" name="Category_rdo" value="<%=rowCnt%>"></td>
...
%>
  ...
  <div align="center">
  <posui:showPageSet infoName="CategoryGrResult" formName="document.forms[0]" curPageName="curPageNum"/>
  </div>
          </td>
        </tr>
      </table>
