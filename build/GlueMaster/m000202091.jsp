...
<%@ page import="com.posdata.glue.master.easyaccess.returnType.PosDecisionRuleVO" %>
 ...
 */
    ...
    PosRowSet rowSet = (PosRowSet)ctx.get("TbM00Rules020_InCnt");
    int inCnt = 20;
    if(rowSet!=null&&rowSet.hasNext()){
        inCnt = Integer.parseInt(""+rowSet.next().getAttribute("CNT"));
    }
    ...
    try
    {
        ...
        if("EXEC".equals(cmd))
        {
            ...
            if(m_Data.size()>0)
            {
                ...
                ruleVO = EasyAccess.getPosDecisionRuleLov(mdlDefineNm,para,startDateEffective);
            }
        }
    }
    catch(Exception e)
...
<title><%=PosContext.getResourceMessage("GMResource","gm.title.069",null,locale)%></title>
  ...
  <tr>
    <td valign=top>
      <table width="930" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr height=25>
          <td><div id=gm_title style="font-family:Verdana;font-size:16px;font-weight:bold;color:000033"><%=PosContext.getResourceMessage("GMResource","gm.title.069",null,locale)%></div></td>
        </tr>
        <tr>
...
<input type="hidden" name="cmd" value="EXEC">
            <table width="100%" border=0 cellspacing="0" cellpadding="0">
              <tr>
                <td><posui:showSelectList name="changePage" staticNames="GEN|DECISIONLOV|MDECISIONMULTI" staticValues="업무기준|판단기준LOV|판단기준멀티" defaultSelectionValues="DECISIONLOV" script="onChange='doChangePage()'"/></td>
                <td align="right">
        ...
        <tr>
          <td>
            <table width="100%" border=1 cellspacing=0 cellpadding=0 bordercolorlight=66666 bordercolordark=ffffff>
              <tr>
                <td class=tbldb>기준정보ID</td></td>
                <td><input type="text" name="MdRuleNm" value="<%= mdlDefineNm%>" class=adb1 onChange="getRuleInfo()" >
                  : <%=ctx.get("MD_RULE_EXPLAIN")==null?"존재하지 않는 기준입니다.":ctx.get("MD_RULE_EXPLAIN")%>
                </td>
              </tr>
              <tr>
                <td class=tbldb><%=PosContext.getResourceMessage("GMResource","gm.label.0199",null,locale)%></td></td>
                <td><%
                rowSet = (PosRowSet)ctx.get("TbM00Rule010");
                if(rowSet != null && rowSet.hasNext()){
%>
                  <posui:showSelectList infoName="TbM00Rule010" name="startDateEffective" nameAttribute="START_ACTIVE_DATE_STR" valueAttribute="START_ACTIVE_DATE" totalName="-" totalValue="-------------------" defaultSelectionValues="<%=startDateEffective%>" style="width:118px" script="onChange='getRuleInfo()'"/><%
                }else{
                ...
                <td><%    if(ruleVO!=null){
                  ...
                  <table width=<%=ruleVO.getColumnCount()*60%> border=1 cellspacing=0 cellpadding=0 bordercolorlight=66666 bordercolordark=ffffff><%
                            for(int j=0;j<ruleVO.getRecordCount();j++)
                            {
                              ...
                              %>
                    <tr class=tblcw><%
                                for(int i=0, iz=ruleVO.getKeyCnt(); i<iz; i++){
                      %>
                      <td><%= ruleVO.getRuleValueAt(i)==null?"&nbsp;":ruleVO.getRuleValueAt(i) %></td><%
                                }
                                for(int i=ruleVO.getKeyCnt(), iz=ruleVO.getColumnCount(); i<iz; i++)
                                {
                      %>
