...
/*
 * @FileName      : 업무기준 목록 >> 업무기준 기본속성 등록 >> 업무기준 데이터 조회(연관판단) >> 업무기준 레이아웃 조회(판단)(Import)
 *                  업무기준 목록 >> 업무기준 기본속성 등록 >> 업무기준 데이터 조회(조건판단) >> 업무기준 레이아웃 조회(판단)(Import)
 *                  업무기준 목록 >> 업무기준 기본속성 등록 >> 업무기준 레이아웃 조회(판단)(Import)
 * Open Issues    :
 * Change history 
 * @2008-07-07 류진영 1.0 최초 생성
 */
...
<title>[import] <%=PosContext.getResourceMessage("GMResource","gm.title.059",null,locale)%></title>
...
function reloadOpener(){
    if(window.opener.document.form_import.UseTp){
        window.opener.document.forms[0].action='m000202060.do';
        window.opener.document.forms[0].ServiceName.value="m000202060-service";
        window.opener.document.forms[0].submit();
    }else{
        window.opener.document.forms[0].action = window.opener.document.forms[0].action+"?inputLayout=true";
        window.opener.document.forms[0].submit();
    }
    self.close();
}
  ...
  <tr>
    <td valign=top>
      <table width="460" border="0" cellspacing="0" cellpadding="0" align="center">
        <tr height=25>
          <td><div id=gm_title style="font-family:Verdana;font-size:16px;font-weight:bold;color:000033">[import] <%=PosContext.getResourceMessage("GMResource","gm.title.059",null,locale)%></div></td>
        </tr>
        <tr>
          <td background="img/gm0006img.gif"><img src="img/gm0007img.gif" width=1 height=8></td>
        </tr>
<form name="form_import" method="post" enctype="multipart/form-data" action="m000202060.do">
              ...
              <tr align="center">
                <td align="left"><%
                  ...
                  if(result != null)
                  {
                      ...
                      out.println("<br>■ 삭제데이타수    : "+result.get("DeleteCnt")+"<br>");
...
%>
        ...
        <tr>
          <td><br><br><div id=gm_import_text><font color="red">
            ...
            <br>■ <%=PosContext.getResourceMessage("GMResource","gm.msg.0086",new Object[]{"5"},locale)%><!-- 항상 5번째 Row부터 데이타를 작성한다. -->
        ...
        <tr>
          <td align="center"><br><% if(isAdmin){ %>
            ...
            <img src="<%=PosContext.getResourceMessage("GMResource","gm.img.close",null,locale)%>" onclick="reloadOpener()" style="cursor:pointer">
          </td>
        </tr>
      </table>
<input type="hidden" name="MdRuleNm" value='<%= request.getParameter("MdRuleNm")%>'>
    </td>
  </tr>
</table>
</form>
...
<input type="hidden" name="MdRuleId" value='<%=request.getParameter("MdRuleId")%>'>
</form>
</body>
</html>